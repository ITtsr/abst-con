...
# クライアントを作成（環境変数 OPENAI_API_KEY を自動使用）
client = OpenAI()
...
def summarize_with_chatgpt(text):
    ...
    try:
        response = client.chat.completions.create(
            model="ft:gpt-3.5-turbo-0125:personal::BtVcdN3V",  # あなたのファインチューンモデル名
            messages=[
                {
                    "role": "user",
...
""" 問題文：平行四辺形ABCDの辺AD,BC上にED=FBとなる点E,Fをとる。このときAF=CEとなることを証明しなさい。 
